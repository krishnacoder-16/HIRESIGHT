...
from services.reports.utils import paginate_dataframe, sort_dataframe, build_report_response, create_csv_export
...
def get_joined_timeline_data(
    ...
    df = get_filtered_timeline_df(timeline_type, month, week, "joining date")
    if df.empty:
        return build_report_response([], {"page": 1, "pageSize": page_size, "totalRecords": 0, "totalPages": 0}, meta={"hasDataset": True})

    # Only include joined candidates
    df = df[df['is_joined'] == True]
    ...
    for _, row in df.iterrows():
        ...
        results.append({
            ...
            "joiningDate": str(row.get('joining date', 'N/A')),
        })
...
def export_joined_timeline_csv(
    search: Optional[str] = None,
    sort_by: Optional[str] = None,
    sort_desc: bool = False,
    timeline_type: Optional[str] = None,
    ...
    export_df['Joining Date'] = df['joining date'].fillna('N/A')
