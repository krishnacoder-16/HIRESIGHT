...
def _get_joined_candidates_df(search: str = None) -> pd.DataFrame:
    df = store.state.get("normalized_dataframe")
    if df is None:
        return pd.DataFrame()

    joined_df = df[df['is_joined']].copy()
    if joined_df.empty:
        return pd.DataFrame()
    ...
    for _, row in joined_df.iterrows():
        records.append({
            ...
            "joiningDate": str(row.get('joining date', 'N/A')) if pd.notna(row.get('joining date')) else 'N/A'
        })
...
def export_joined_candidates(search: str = None, sort_by: str = None, sort_desc: bool = False):
    ...
    return create_csv_export(df, cols_map, "hiresight_joined_candidates.csv")
