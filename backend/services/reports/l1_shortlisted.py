...
def _get_l1_shortlisted_df(search: str = None) -> pd.DataFrame:
    df = store.state.get("normalized_dataframe")
    if df is None:
        return pd.DataFrame()

    # Find columns matching 'l1'
    target_cols = [c for c in df.columns if 'l1' in c.lower()]
    if not target_cols:
    ...
    for col in target_cols:
        mask = mask | df[col].astype(str).str.contains('shortlisted|l1 cleared|selected', case=False, na=False)
...
def get_l1_shortlisted(page: int, page_size: int, search: str = None, sort_by: str = None, sort_desc: bool = False):
    df = _get_l1_shortlisted_df(search)
...
def export_l1_shortlisted(search: str = None, sort_by: str = None, sort_desc: bool = False):
    df = _get_l1_shortlisted_df(search)
    if df.empty:
    ...
    return create_csv_export(df, cols_map, "hiresight_l1_shortlisted.csv")
